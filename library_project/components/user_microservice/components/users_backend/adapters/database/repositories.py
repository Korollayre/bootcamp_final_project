...
class UsersRepo(BaseRepository, interfaces.UsersRepo):
    ...
    def get_by_id(self, user_id: int) -> Optional[Users]:
        return self.session.query(Users).filter(Users.id == user_id
                                                ).one_or_none()

    def get_by_email(self, email: str) -> Optional[Users]:
        return self.session.query(Users).filter(Users.email == email
                                                ).one_or_none()
    ...
    def login(self, user_mail: str, user_password: str) -> Optional[Users]:
        return self.session.query(Users).filter(
            Users.email == user_mail, Users.password == user_password
        ).one_or_none()
